...
import { createCampaign } from '../lib/google-ads.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // ─────────────────────────────────────────────────────────
      // ADS-KAMPAGNE (Vorkasse vom Makler) — ZUERST abfangen!
      // Sonst würde diese Zahlung fälschlich als Abo verbucht.
      // ─────────────────────────────────────────────────────────
      if (meta.type === 'ads_campaign') {
        try {
          const result = await createCampaign({
            customerId:          process.env.GOOGLE_ADS_TEST_CUSTOMER_ID, // live: Konto des Maklers
            campaignName:        meta.campaignName,
            dailyBudgetEuros:    Number(meta.dailyBudgetEuros || 20),
            keywords:            meta.keywords ? JSON.parse(meta.keywords) : [],
            geoTargetConstantId: meta.geoTargetConstantId || '2276',
            finalUrl:            meta.finalUrl
          });
          console.log('✅ Ads-Kampagne angelegt:', JSON.stringify(result));
        } catch (e) {
          // Zahlung ist da, Kampagne scheiterte → hier loggen / Makler benachrichtigen
          console.error('❌ Ads-Kampagne fehlgeschlagen:', e.message);
        }
        return res.status(200).json({ received: true }); // fertig — KEINE Plan-Logik
      }
